...
class TextCNN(nn.Module):
    def __init__(self, n_filters=64 * 2, filter_sizes=[3, 4, 5, 6], dropout=0.6):
...
class BertForSequenceClassification_textCNN_with2seq(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
        ...
        self.init_weights()
        # self.textcnn = TextCNN(n_filters=config.filter_num, filter_sizes=config.filter_size, dropout=0.6)
        self.my_fc = nn.Sequential(nn.Linear(768 * 2,768), my_mish(),
                                   nn.Dropout(),
                                   nn.Linear(768, self.config.num_labels)
                                  )
        self.ban = BANLayer(768, 768, 768, 768)
    ...
    def forward(
        ...
        cls_feature1 = hidden_states1[-1][:, 0, :]  # [bs, hidden]  # [batch, 768]
        cls_feature2 = hidden_states2[-1][:, 0, :]  # [bs, hidden]  # [batch, 768]

        # print(cls_feature1.size())
        # print(cls_feature2.size())


        # logits = self.my_fc(torch.cat([cls_feature1, cls_feature2], dim=1))

        # ban
        cls1 = cls_feature1.unsqueeze(1)
        cls2 = cls_feature2.unsqueeze(1)
        logits = self.my_fc_ban(self.ban(cls1, cls2))  # [batch, 768]
